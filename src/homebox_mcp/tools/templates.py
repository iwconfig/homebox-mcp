...
async def handle_create_template(
    ...
    payload.update({"defaultInsured": defaultInsured, "defaultLifetimeWarranty": defaultLifetimeWarranty})
    if defaultWarrantyDetails: payload["defaultWarrantyDetails"] = defaultWarrantyDetails
    payload.update({"includePurchaseFields": includePurchaseFields, "includeSoldFields": includeSoldFields, "includeWarrantyFields": includeWarrantyFields})
...
async def handle_create_item_from_template(client: HomeboxClient, id: str, name: str, locationId: str, quantity: int = 1, description: str | None = None, labelIds: list[str] | None = None) -> str:
    payload = {"name": name, "locationId": locationId, "quantity": quantity}
    if description: payload["description"] = description
