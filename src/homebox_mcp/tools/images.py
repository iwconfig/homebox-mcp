...
async def _apply_image_ops(image_data: bytes, crop_box: list[int] | None = None, rotation: int | None = None) -> bytes:
    ...
    if crop_box:
        w, h = img.size
        left, top, right, bottom = [int(crop_box[i] * [w, h, w, h][i] / 1000) for i in range(4)]
...
async def handle_finalize_processed_item(
    ...
    if source == "local":
        ...
        create_payload = {"name": name, "locationId": locationId, "description": description or "", "labelIds": labelIds or []}
        ...
        update_payload.update({
            "manufacturer": manufacturer or "", "modelNumber": modelNumber or "",
            "serialNumber": serialNumber or "", "notes": notes or "",
            "purchaseTime": "0001-01-01T00:00:00Z", "warrantyExpires": "0001-01-01T00:00:00Z"
        })
        ...
        item = await client.request("GET", f"items/{id}")
        update_payload = item.copy()
        if "location" in item and item["location"]: update_payload["locationId"] = item["location"]["id"]
        if "labels" in item and item["labels"]: update_payload["labelIds"] = [l["id"] for l in item["labels"]]
        update_payload.update({
        ...
        for key in ["purchaseTime", "soldTime", "warrantyExpires"]:
            if not update_payload.get(key): update_payload[key] = "0001-01-01T00:00:00Z"
...
async def handle_crop_item_image(client: HomeboxClient, item_id: str, attachment_id: str, crop_box: list[int]) -> dict[str, Any]:
    ...
    if not existing: raise ValueError(f"Attachment {attachment_id} not found")
...
async def handle_split_item_from_image(
    ...
    if source == "local":
        ...
        with open(path, "rb") as f: source_data = f.read()
    else:
        if not attachment_id:
            item = await client.request("GET", f"items/{id}")
            attachments = item.get("attachments", [])
    ...
    results, total = [], len(extracted_objects)
    for i, obj in enumerate(extracted_objects):
        ...
        create_payload = {"name": obj["name"], "locationId": obj["locationId"], "description": obj.get("description", ""), "labelIds": obj.get("labelIds", [])}
        ...
        update_payload.update({"notes": obj.get("notes", ""), "purchaseTime": "0001-01-01T00:00:00Z", "warrantyExpires": "0001-01-01T00:00:00Z"})
        await client.request("PUT", f"items/{new_id}", json=update_payload)
        results.append(new_id)
    if ctx: await ctx.report_progress(total, total, message="All items extracted.")
    if source == "local": os.remove(os.path.join(INBOX_DIR, id))
    else: await client.request("DELETE", f"items/{id}")
