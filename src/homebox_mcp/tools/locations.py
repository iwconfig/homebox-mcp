...
def register_locations_tools(mcp: FastMCP, client: HomeboxClient):
    ...
    async def list_locations(
        filter_children: Annotated[bool, "Whether to filter children"] = False
    ) -> dict:
        """Get All Locations"""
    ...
    async def create_location(
        ...
        """Create Location"""
        return await handle_create_location(
            client, name=name, description=description, parent_id=parent_id
        )

    @mcp.tool(output_schema={"type": "object"})
    async def get_locations_tree(
        with_items: Annotated[bool, "Whether to include items in the tree"] = False
    ) -> dict:
        """Get Locations Tree"""
    ...
    async def update_location(
        ...
        return await handle_update_location(
            client, id=id, name=name, description=description, parent_id=parent_id
        )
