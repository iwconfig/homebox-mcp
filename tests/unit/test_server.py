...
def test_server_startup_stdio(monkeypatch):
    ...
    with patch.object(sys, "argv", ["homebox-mcp"]), patch("homebox_mcp.server.mcp") as mock_mcp:
        main()
...
def test_server_startup_sse_arg(monkeypatch):
    ...
    with patch.object(sys, "argv", ["homebox-mcp", "sse"]), patch("homebox_mcp.server.mcp") as mock_mcp:
        main()
...
def test_server_startup_explicit_stdio_arg(monkeypatch):
    ...
    with patch.object(sys, "argv", ["homebox-mcp", "stdio"]), patch("homebox_mcp.server.mcp") as mock_mcp:
        main()
        mock_mcp.run.assert_called_once_with(transport="stdio")
