...
async def test_logging_middleware(test_mcp_with_middleware, caplog):
    ...
    assert any(
        "tools/call" in r.message or "ping" in r.message for r in caplog.records if r.name == "test_middleware_logger"
    )
...
async def test_error_handling_middleware(test_mcp_with_middleware):
    ...
    async with Client(test_mcp_with_middleware) as client:
        ...
        # If the server returns an error, it might be in content or raise an exception
        # depending on transport/client logic.
