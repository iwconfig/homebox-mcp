...
async def handle_update_user_self(client: HomeboxClient, name: str | None = None, email: str | None = None) -> str:
    ...
    if email and email != user_item.get("email"): check_user_protection(user_item, "change_email")
    payload = {"name": name or user_item.get("name"), "email": email or user_item.get("email")}
...
async def handle_change_password(client: HomeboxClient, current: str, new: str) -> str:
    ...
    try:
        ...
        if "404" in str(e): return "Change password endpoint not found (404). This might not be supported in this Homebox version."
        raise
