    ...
    async with respx.mock(base_url="http://mock-homebox/api/v1") as respx_mock:
        respx_mock.post("/users/login").mock(
            return_value=Response(
                200,
                json={"token": "Bearer new-token", "expiresAt": (datetime.now(UTC) + timedelta(hours=1)).isoformat()},
            )
        )
...
async def test_token_auto_refresh(client):
    ...
    async with respx.mock(base_url="http://mock-homebox/api/v1") as respx_mock:
        # Mock refresh call
        refresh_route = respx_mock.get("/users/refresh").mock(
            return_value=Response(
                200,
                json={
...
async def test_request_image_base64(api_key_client):
    async with respx.mock(base_url="http://mock-homebox/api/v1") as respx_mock:
        ...
        respx_mock.get("/img").mock(return_value=Response(200, content=img_data, headers={"Content-Type": "image/png"}))
...
async def test_request_plain_text(api_key_client):
    async with respx.mock(base_url="http://mock-homebox/api/v1") as respx_mock:
        respx_mock.get("/text").mock(
            return_value=Response(200, text="Hello World", headers={"Content-Type": "text/plain"})
        )
...
async def test_request_error_handling(api_key_client):
    ...
    async def test_request_json_decode_error(api_key_client):
        ...
        async with respx.mock(base_url="http://mock-homebox/api/v1") as respx_mock:
            ...
            respx_mock.get("/ok-but-bad-json").mock(
